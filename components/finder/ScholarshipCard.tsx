...
export function ScholarshipCard({ result, destinations }: { result: Scholarship; destinations: Option[] }) {
  ...
  return <article className="result-card"><div className="result-top"><span className={`status status-${result.status_detail}`}>{result.status_detail.replaceAll('_', ' ')}</span><span className={`fit fit-${result.fit}`}>{result.fit === 'confirmed' ? 'Matches your criteria' : 'Possible match'}</span></div><h3>{result.name}</h3><p className="provider">{result.provider}</p><div className="result-meta"><span>â†³ {destination}</span><span>â—· Verified {formatDate(result.last_verified_at)}</span></div>{result.eligibility_note && <p className="eligibility"><strong>Eligibility note:</strong> {result.eligibility_note}</p>}<a className="official-link" href={result.official_url} target="_blank" rel="noreferrer">View official scholarship <span aria-hidden="true">â†—</span></a></article>
}