...
export function Finder() {
  ...
  async function submit(event: FormEvent, cursor?: string) {
    ...
    if (!form.destinations.length) next.destinations = 'Select at least one destination.'
    if (!form.degree) next.degree = 'Select what you are applying for.'
    setValidation(next); if (Object.keys(next).length) return
    setLoading(true); setError(null)
    const input: SearchInput = { origin_country: form.origin, program_level: form.degree, target_countries: form.destinations, limit: 20, ...(form.field ? { field: form.field } : {}), ...(cursor ? { cursor } : {}) }
  ...
  return <div className="finder-app"><SiteHeader /><section className="finder-hero"><h1>Let’s find scholarships that fit you.</h1><p>Get a shortlist of verified funding opportunities for your Master&apos;s or PhD—without spending hours chasing outdated links.</p><div className="hero-points"><span>✓&nbsp; No sign up required</span><span>✓&nbsp; Free to use</span></div></section><FinderStepper step={searched ? 2 : 1} /><main className="finder-main">
    {!searched && <section className="form-panel" aria-labelledby="form-heading"><form onSubmit={submit} noValidate>
      <label className="form-question">Where are you from? <select value={form.origin} onChange={(e) => update('origin', e.target.value)} aria-invalid={!!validation.origin}><option value="">Search for your country</option>{taxonomies?.countries.map((item) => <option value={item.code} key={item.code}>{item.label}</option>)}</select><small>Helps us identify scholarships you&apos;re eligible to apply for</small>{validation.origin && <em className="error-text">{validation.origin}</em>}</label>
      <fieldset className="destination-control"><legend>Where do you want to study?</legend><div className="option-grid">{taxonomies?.destinations.map((item) => <label className={`option-chip ${form.destinations.includes(item.code) ? 'selected' : ''}`} key={item.code}><input type="checkbox" checked={form.destinations.includes(item.code)} onChange={() => toggle(item.code)} />{item.label}</label>)}<label className={`option-chip ${!!taxonomies?.destinations.length && form.destinations.length === taxonomies.destinations.length ? 'selected' : ''}`}><input type="checkbox" checked={!!taxonomies?.destinations.length && form.destinations.length === taxonomies.destinations.length} onChange={(event) => update('destinations', event.target.checked ? (taxonomies?.destinations.map((item) => item.code) ?? []) : [])} />I&apos;m open to any of these</label><label className={`option-chip ${showOtherDestination ? 'selected' : ''}`}><input type="checkbox" checked={showOtherDestination} onChange={(event) => { const checked = event.target.checked; setShowOtherDestination(checked); if (!checked) { setOtherDestination(''); setForm((current) => ({ ...current, destinations: otherDestination ? current.destinations.filter((code) => code !== otherDestination) : current.destinations })) } }} />Somewhere else</label></div>{showOtherDestination && taxonomies && <CountryCombobox options={taxonomies.countries} value={otherDestination} onChange={(code) => { setOtherDestination(code); setForm((current) => ({ ...current, destinations: current.destinations.includes(code) ? current.destinations : [...current.destinations, code] })) }} />}{validation.destinations && <em className="error-text">{validation.destinations}</em>}</fieldset>
