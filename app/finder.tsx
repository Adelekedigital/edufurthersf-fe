...
export function Finder({ searchId, selectedScholarshipId }: { searchId?: string; selectedScholarshipId?: string }) {
  ...
  useEffect(() => {
    if (!searchId || !taxonomies || restoredSearch.current === searchId) return
    ...
    getSavedSearch(searchId).then((response) => {
      ...
      if (filters) {
        const destinationCodes = new Set((taxonomies?.destinations ?? []).map((item) => item.code))
        const selectedDestinations = filters.target_countries.filter((code) => destinationCodes.has(code))
        const alternateDestinations = filters.target_countries.filter((code) => !destinationCodes.has(code))
        setForm({ origin: filters.origin_country, destinations: selectedDestinations, field: filters.field ?? '', degree: filters.program_level })
        setOtherDestinations(alternateDestinations)
        setShowOtherDestination(alternateDestinations.length > 0)
  ...
  async function submit(event: FormEvent, cursor?: string) {
    ...
    try {
      ...
      if (!cursor && returnedSearchId) { cacheSearchResponse(returnedSearchId, response); writeSearchProfile(returnedSearchId, input); setActiveSearchId(returnedSearchId); setRoutePending(true); router.push('/search/' + encodeURIComponent(returnedSearchId)) }
  ...
  const openDetails = (identifier: string) => { if (!activeSearchId) return; setPendingModalId(identifier); modalOpenedFromResults.current = true; router.push('/search/' + encodeURIComponent(activeSearchId) + '?scholarship=' + encodeURIComponent(identifier)) }
  const closeDetails = () => { setPendingModalId(null); if (modalOpenedFromResults.current) { modalOpenedFromResults.current = false; router.back() } else if (activeSearchId) router.replace('/search/' + encodeURIComponent(activeSearchId)) }
  const refineSearch = () => {
    const filters = activeSearchId ? readSearchProfile(activeSearchId) : null
    if (filters) {
      const destinationCodes = new Set((taxonomies?.destinations ?? []).map((item) => item.code))
      const selectedDestinations = filters.target_countries.filter((code) => destinationCodes.has(code))
      const alternateDestinations = filters.target_countries.filter((code) => !destinationCodes.has(code))
    ...
    setSearched(false); setResults([]); setNextCursor(null); setWarnings([]); setError(null); setActiveSearchId(''); setRoutePending(false); router.replace('/')
  }

  const initialSearchPending = (loading && !searched) || routePending
  ...
  return <div className="finder-app"><SiteHeader /><section className="finder-hero"><h1>Let&apos;s find scholarships that fit you.</h1><p>Get a shortlist of verified funding opportunities for your Master&apos;s or PhD&mdash;without spending hours chasing outdated links.</p><div className="hero-points"><span>&#10003;&nbsp; No sign up required</span><span>&#10003;&nbsp; Free to use</span></div></section><FinderStepper step={searched ? 2 : 1} /><main className="finder-main">
    {initialSearchPending && <section className="search-loading" role="status" aria-live="polite" aria-busy="true"><div className="loading-spinner" aria-hidden="true" /><h2>Finding scholarships…</h2><p>We&apos;re matching your answers with verified opportunities.</p><div className="loading-skeleton" aria-hidden="true"><i /><i /><i /></div></section>}
    {!initialSearchPending && !searched && <section className="form-panel" aria-labelledby="form-heading"><form onSubmit={submit} noValidate>
