...
export function Finder({ searchId, selectedScholarshipId }: { searchId?: string; selectedScholarshipId?: string }) {
  ...
  async function submit(event: FormEvent, cursor?: string) {
    ...
    const input: SearchInput = { origin_country: form.origin, program_levels: form.degree, target_countries: targetCountries, limit: 20, ...(form.field ? { field: form.field } : {}), ...(cursor ? { cursor } : {}) }
    const thisRequest = ++requestId.current
